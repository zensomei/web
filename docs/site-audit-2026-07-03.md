...
2026-07-03の追加作業で、公開用CVは以下に統一しました。

- `documents/cv.pdf`

統一前は以下2つのPDFがGit上で追跡されていました。

- `documents/CV_somei.pdf`
- `documents/CV_Somei.pdf`

この2ファイルは大文字小文字だけが異なり、Windows/macOSの通常のcase-insensitiveなファイルシステムでは同時に安全にcheckoutできませんでした。実体確認では、`CV_somei.pdf` は653,933 bytes / blob `0fdda91bfad8104a9a452e663df41280cc8f3062`、`CV_Somei.pdf` は649,873 bytes / blob `84b84ebd600962abfc5963fa596a25e666e58f29` で、同一ファイルではありませんでした。

現行HTMLが参照していた `documents/CV_somei.pdf` を公開用として採用し、同じ内容を `documents/cv.pdf` に配置しました。`index.html` のCVリンクも `documents/cv.pdf` に更新済みです。旧PDF URL (`documents/CV_somei.pdf`, `documents/CV_Somei.pdf`) の互換維持はしていません。
...
- case-onlyのCVファイル名重複は2026-07-03に解消済み。公開用CVは `documents/cv.pdf`。
...
- case-onlyのCVファイル名重複は解消済み。旧PDF URLの互換維持はしていないため、必要であれば移行後に別途リダイレクト方針を検討する。
...
- case-onlyのCVファイル名重複は解消済み。ただし、今後もcase-only差分のファイル名を追加しないよう注意する。
...
2. コンテンツデータの初期スキーマを作る。
   - profile
   - publications
   - experiences
   - awards
   - works/news
3. `index.html` からコンテンツを抽出し、YAML/Markdownの初期データに落とす。
4. Astroプロジェクトを別ブランチで作成し、既存URL互換を保ちながら静的buildできるようにする。
5. 最低限のCIを追加する。
   - install
   - build
   - link/checkまたはHTML check
6. README / CONTRIBUTING / update template / Issue templateを整備する。
7. モバイル表示をPlaywright等で確認する。
8. デザイン刷新は、情報設計とデータ分離が済んだ後に行う。
